...
depart=20 #population de départ
...
class humain:
    ...
    def react_poteau(self):
        distx = poteau[0] - self.x
        disty = poteau[1] - self.y
        distp = (distx**2+disty**2)**(1/2)
        if (distp < rayon_poteau + rayon + 10):
            self.x -= distx * 0.1
            self.vx *= 0.6
            self.vy -= disty * 0.1
            self.vy *= 0.6
...
while continuer == True:
    ...
    #py.draw.circle(fenetre,(0,0,0),poteau,rayon_poteau)
